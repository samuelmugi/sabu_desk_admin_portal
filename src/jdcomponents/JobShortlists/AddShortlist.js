...
const AddShortlist = (props) => {
    ...
    const [userValuesErrors, setShortlistValuesErrors, userValuesErrorsRef] = useState({
        username: '',
        email: '',
        firstName: '',
        lastName: '',
    ...
    const [userValues, setShortlistValues, userValuesRef] = useState(props?.valueIntoModal);
    useEffect(() => {
        (async function () {
    ...
    const changeHandler = (e) => {
        setShortlistValues((prevValues) => {
            return {...prevValues, [e.target.name]: e.target.value};
        });
    };
    const updateShortlistValues = (name, value) => {
        setShortlistValues((prevValues) => {
    ...
    const validateValues = () => {
        ...
        Object.keys(userErrors).map((key) => {
            if (user[key] === null || user[key] === '' || user[key] === undefined) {
                ...
                setShortlistValuesErrors((prevValues) => {
    ...
    const fetchADDetails = async () => {
        ...
        try {
            ...
            updateShortlistValues('username', username);

            const firstname = data?.firstname;
            updateShortlistValues('firstName', firstname);

            const email = data?.email;
            updateShortlistValues('email', email);

            const lastname = data?.lastname;
            updateShortlistValues('lastName', lastname);
    ...
    const handleRoleChange = async (e, {value}) => {
        ...
        updateShortlistValues('userRole', JSON.parse(value));
    };
    ...
    return (
        <Aux>
            <React.Fragment>
                <Popup
                    content={props?.action === 'add' ? "Add Shortlist" : "Edit Shortlist"}
                    trigger={
                        (props?.action === 'add'
                ...
                >
                    <DialogTitle
                        id="max-width-dialog-title">{props?.action === 'add' ? "Add Shortlist" : "Edit Shortlist"}</DialogTitle>
                    <DialogContent>
                        <Row>
                            <Col>
                                <Form>
                                    <Form.Group>
                                        <Form.Input
                                            label="Windows Shortlistname"
                                            placeholder="Windows Shortlistname"
                                            name="username"
                                        ...
                                        <Form.Button
                                            positive
                                            label="Fetch A.D. Shortlist Details..."
                                            content="Fetch..."
