...
const AddInterview = (props) => {
    ...
    const [userValuesErrors, setInterviewValuesErrors, userValuesErrorsRef] = useState({
        username: '',
        email: '',
        firstName: '',
        lastName: '',
    ...
    const [userValues, setInterviewValues, userValuesRef] = useState(props?.valueIntoModal);
    useEffect(() => {
        (async function () {
    ...
    const changeHandler = (e) => {
        setInterviewValues((prevValues) => {
            return {...prevValues, [e.target.name]: e.target.value};
        });
    };
    const updateInterviewValues = (name, value) => {
        setInterviewValues((prevValues) => {
    ...
    const validateValues = () => {
        ...
        Object.keys(userErrors).map((key) => {
            if (user[key] === null || user[key] === '' || user[key] === undefined) {
                ...
                setInterviewValuesErrors((prevValues) => {
    ...
    const fetchADDetails = async () => {
        ...
        try {
            ...
            updateInterviewValues('username', username);

            const firstname = data?.firstname;
            updateInterviewValues('firstName', firstname);

            const email = data?.email;
            updateInterviewValues('email', email);

            const lastname = data?.lastname;
            updateInterviewValues('lastName', lastname);
    ...
    const handleRoleChange = async (e, {value}) => {
        ...
        updateInterviewValues('userRole', JSON.parse(value));
    };
    ...
    return (
        <Aux>
            <React.Fragment>
                <Popup
                    content={props?.action === 'add' ? "Add Interview" : "Edit Interview"}
                    trigger={
                        (props?.action === 'add'
                ...
                >
                    <DialogTitle
                        id="max-width-dialog-title">{props?.action === 'add' ? "Add Interview" : "Edit Interview"}</DialogTitle>
                    <DialogContent>
                        <Row>
                            <Col>
                                <Form>
                                    <Form.Group>
                                        <Form.Input
                                            label="Windows Interviewname"
                                            placeholder="Windows Interviewname"
                                            name="username"
                                        ...
                                        <Form.Button
                                            positive
                                            label="Fetch A.D. Interview Details..."
                                            content="Fetch..."
