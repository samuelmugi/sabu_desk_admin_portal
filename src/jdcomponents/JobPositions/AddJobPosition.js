...
const AddPosition = (props) => {
    ...
    const [userValuesErrors, setPositionValuesErrors, userValuesErrorsRef] = useState({
        username: '',
        email: '',
        firstName: '',
        lastName: '',
    ...
    const [userValues, setPositionValues, userValuesRef] = useState(props?.valueIntoModal);
    useEffect(() => {
        (async function () {
    ...
    const updatePositionValues = (name, value) => {
        setPositionValues((prevValues) => {
    ...
    const validateValues = () => {
        ...
        Object.keys(userErrors).map((key) => {
            if (user[key] === null || user[key] === '' || user[key] === undefined) {
                ...
                setPositionValuesErrors((prevValues) => {
    ...
    const fetchADDetails = async () => {
        ...
        try {
            ...
            updatePositionValues('username', username);

            const firstname = data?.firstname;
            updatePositionValues('firstName', firstname);

            const email = data?.email;
            updatePositionValues('email', email);

            const lastname = data?.lastname;
            updatePositionValues('lastName', lastname);
    ...
    const handleRoleChange = async (e, {value}) => {
        ...
        updatePositionValues('userRole', JSON.parse(value));
    };
    ...
    return (
        <Aux>
            <React.Fragment>
                <Popup
                    content={props?.action === 'add' ? "Add Position" : "Edit Position"}
                    trigger={
                        (props?.action === 'add'
                ...
                >
                    <DialogTitle
                        id="max-width-dialog-title">{props?.action === 'add' ? "Add Position" : "Edit Position"}</DialogTitle>
                    <DialogContent>
                        <Row>
                            <Col>
                                <Form>
                                    <Form.Group>
                                        <Form.Input
                                            label="Windows Positionname"
                                            placeholder="Windows Positionname"
                                            name="username"
                                        ...
                                        <Form.Button
                                            positive
                                            label="Fetch A.D. Position Details..."
                                            content="Fetch..."
